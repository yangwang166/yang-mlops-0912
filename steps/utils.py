...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"yang-mlops-project-0912-model{resource_name_suffix}"
    else:
        # Read ml model name from databricks-config
        return _get_ml_config_value(env, "yang-mlops-project-0912_model_name")
...
def set_experiment(env, test_mode=False):
    ...
    if env == "dev":
        ...
        # to eliminate the need to create a directory named '/yang-mlops-project-0912'
        # to run ML code.
        experiment_name = (
            f"/yang-mlops-project-0912-experiment{resource_name_suffix}"
        )
    elif test_mode:
        experiment_name = f"/yang-mlops-project-0912/yang-mlops-project-0912-experiment{resource_name_suffix}"
    else:
        # Read mlflow experiment name from databricks-config
        experiment_name = _get_ml_config_value(
            env, "yang-mlops-project-0912_experiment_name"
        )
