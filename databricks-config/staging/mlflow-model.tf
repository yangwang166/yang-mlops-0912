...
* [Recurring model training job](https://adb-8405830731420074.14.azuredatabricks.net#job/${databricks_job.model_training_job.id}): trains fresh model versions using the latest ML code.
...
* [Recurring batch inference job](https://adb-8405830731420074.14.azuredatabricks.net#job/${databricks_job.batch_inference_job.id}): applies the latest ${local.env} model version for batch inference.
EOF
}
