...
![MLOps project template diagram](../docs/images/mlops-resource-config.png)
...
Then, resolve the TODOS in `staging/batch-job.tf` and `prod/batch-job.tf`, passing
...
resource "databricks_job" "batch_inference_job" {
  name = "yang-mlops-project-0912 batch inference job"
...
The example above defines a Databricks job with name `yang-mlops-project-0912 batch inference job`
